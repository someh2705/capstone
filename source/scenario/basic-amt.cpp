...
RelayApp::StartApplication()
{
    if (!m_recvSocket)
    {
        ...
        auto local = InetSocketAddress(m_multicastGroup, m_multicastPort);
...
void
GatewayApp::Setup(uint16_t unicastPort, Ipv4Address multicastGroup, uint16_t multicastPort)
{
...
GatewayApp::HandleRead(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->RecvFrom(from)))
    {
        ...
        m_sendSocket->SendTo(packet, 0, InetSocketAddress(m_multicastGroup, m_multicastPort));
    }
}
